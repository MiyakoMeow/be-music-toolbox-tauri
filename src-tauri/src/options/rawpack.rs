...
pub async fn rawpack_unzip_numeric_to_bms_folder(
    ...
    unzip_numeric_to_bms_folder(&pack_path, &cache_path, &root_path, confirm, replace)
        .await
...
pub async fn rawpack_unzip_with_name_to_bms_folder(
    ...
    let cache_path = PathBuf::from(cache_dir);
    let root_path = PathBuf::from(root_dir);
    unzip_with_name_to_bms_folder(&pack_path, &cache_path, &root_path, confirm, replace)
        .await
        .map_err(|e| e.to_string())
}
